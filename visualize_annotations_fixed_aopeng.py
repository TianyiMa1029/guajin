...
格式 3（Aopeng / instances 风格）
- 根节点包含 `instances`
- 类别字段通常为 `category / categoryName`
- 框坐标通常在 `instances[*].children[*].cameras[*].frames[*].shape`
- 图像信息通常在根节点 `frames[*].frames[*]`
- 示例见 aopeng.json
...
LABEL_ALIASES = {
    ...
    "行人": "pedestrian",
    # two-wheeler
    "lianglunche": "lianglunche",
    "bicycle": "lianglunche",
    "bike": "lianglunche",
    "two_wheeler": "lianglunche",
    "two-wheeler": "lianglunche",
    "两轮车": "lianglunche",
    # car
    "car": "car",
    "小车": "car",
    # small tricycle
    "xiaosanlunche": "xiaosanlunche",
    "tricycle": "xiaosanlunche",
    "小三轮车": "xiaosanlunche",
}
...
def safe_rounded_rectangle(
    draw: ImageDraw.ImageDraw,
    bbox: tuple[int, int, int, int] | tuple[float, float, float, float],
    fill: Optional[tuple[int, int, int, int] | tuple[int, int, int]] = None,
    outline: Optional[tuple[int, int, int, int] | tuple[int, int, int]] = None,
    width: int = 1,
    radius: int = 12,
) -> None:
    """
    Pillow 的 rounded_rectangle 在极窄 / 极矮的框上可能因为内部子矩形反向而抛 ValueError。
    这里统一做坐标排序、半径裁剪，并在必要时自动退化为普通 rectangle。
    """
    ...
    bh = y1 - y0
    if bw <= 0 or bh <= 0:
        # 退化框直接用 rectangle，至少不报错
        draw.rectangle((x0, y0, max(x0, x1), max(y0, y1)), fill=fill, outline=outline, width=width)
        return

    # Pillow 内部使用的是包含式坐标，并会继续拆分圆角区域；
    # 半径需要比短边更保守一些，否则在小框上容易出现内部反向矩形。
    max_r_w = max(0, (bw - 1) // 2)
    max_r_h = max(0, (bh - 1) // 2)
    safe_r = int(min(max_r_w, max_r_h, max(0, radius)))
    ...
    try:
        draw.rounded_rectangle((x0, y0, x1, y1), radius=safe_r, fill=fill, outline=outline, width=width)
    except Exception:
        draw.rectangle((x0, y0, x1, y1), fill=fill, outline=outline, width=width)
...
def draw_shadowed_rounded_rect(
    ...
    safe_rounded_rectangle(draw, shadow_box, radius=radius, fill=(0, 0, 0, shadow_alpha))
    safe_rounded_rectangle(draw, bbox, radius=radius, fill=fill, outline=outline, width=width)
...
def normalize_label_name(label: Any) -> str:
    text = str(label or "unknown").strip()
    if not text:
        return "unknown"
    key = text.casefold().replace(" ", "").replace("-", "_")
    return LABEL_ALIASES.get(key, LABEL_ALIASES.get(text, text))


def basename_from_url(url: Any) -> Optional[str]:
    if not url:
        return None
    try:
...
def box_from_shape_dict(shape: dict[str, Any]) -> Optional[tuple[float, float, float, float]]:
    points = shape.get("points")
    xs: list[float] = []
    ys: list[float] = []
    collect_xy_from_coordinates(points, xs, ys)
    if xs and ys:
        x1, x2 = min(xs), max(xs)
        y1, y2 = min(ys), max(ys)
        if x2 > x1 and y2 > y1:
    ...
    def walk(node: Any) -> None:
        if isinstance(node, dict):
            if (
                ...
                y1, y2 = sorted((y1, y2))
                if x2 > x1 and y2 > y1:
                    label = normalize_label_name(node.get("identity") or node.get("label") or "unknown")
...
def parse_featurecollection_format(data: dict[str, Any]) -> tuple[list[Box], Optional[str], Optional[int], Optional[int], str]:
    ...
    for feat in features:
        ...
        y1, y2 = min(ys), max(ys)
        if x2 > x1 and y2 > y1:
            boxes.append(Box(label=normalize_label_name(label), x1=x1, y1=y1, x2=x2, y2=y2, source_id=str(sid) if sid is not None else None))
...
def parse_aopeng_format(data: dict[str, Any]) -> tuple[list[Box], Optional[str], Optional[int], Optional[int], str]:
    boxes: list[Box] = []
    ...
    root_frames = data.get("frames") or []
    for camera_item in root_frames:
        if not isinstance(camera_item, dict):
            continue
        for frame_meta in camera_item.get("frames", []) or []:
            if not isinstance(frame_meta, dict):
                continue
            if target_frame_index is None and isinstance(frame_meta.get("frameIndex"), int):
                target_frame_index = int(frame_meta.get("frameIndex"))
            if image_name is None:
                image_name = basename_from_url(frame_meta.get("imageUrl")) or frame_meta.get("imageName") or frame_meta.get("imagename")
            if width is None and isinstance(frame_meta.get("imageWidth"), (int, float)):
                width = int(frame_meta.get("imageWidth"))
            if height is None and isinstance(frame_meta.get("imageHeight"), (int, float)):
                height = int(frame_meta.get("imageHeight"))
            if image_name and width and height:
                break
        if image_name and width and height:
            break
    ...
    for inst in instances:
        if not isinstance(inst, dict):
            continue
        label = normalize_label_name(inst.get("categoryName") or inst.get("category") or "unknown")
        inst_id = inst.get("id") or inst.get("number")

        for child in inst.get("children", []) or []:
            if not isinstance(child, dict):
                continue
            child_id = child.get("id") or child.get("number")
            for camera in child.get("cameras", []) or []:
                if not isinstance(camera, dict):
                    continue
                for frame in camera.get("frames", []) or []:
                    if not isinstance(frame, dict):
                        continue
                    if target_frame_index is not None and frame.get("frameIndex") not in (None, target_frame_index):
                        continue
                    shape_type = str(frame.get("shapeType") or "").strip().lower()
                    if shape_type and shape_type not in {"rectangle", "rect"}:
                        continue
                    shape = frame.get("shape") or {}
                    bbox = box_from_shape_dict(shape)
                    if not bbox:
                        continue
                    x1, y1, x2, y2 = bbox
                    sid_parts = [v for v in (inst_id, child_id, frame.get("order")) if v is not None]
                    sid = "_".join(str(v) for v in sid_parts) if sid_parts else None
                    boxes.append(Box(label=label, x1=x1, y1=y1, x2=x2, y2=y2, source_id=sid))

    return boxes, str(image_name) if image_name else None, width, height, "aopeng_instances"
...
def detect_and_parse(data: dict[str, Any]) -> tuple[list[Box], Optional[str], Optional[int], Optional[int], str]:
    ...
    if isinstance(data, dict) and "instances" in data and isinstance(data.get("instances"), list):
        return parse_aopeng_format(data)
...
def draw_boxes_on_image(
    ...
    for idx, box in indexed_boxes:
        ...
        pill_w = tw + pad_x * 2
        pill_h = th + pad_y * 2
        pill_w = max(pill_w, pill_h)
        radius = max(3, min(pill_h // 2, pill_w // 2, 12))
        ...
        px = max(4, min(px, max(4, w - pill_w - 4)))
        py = max(4, min(py, max(4, h - pill_h - 4)))
...
def draw_info_panel(
    ...
    safe_rounded_rectangle(d, (c1[0], c1[1], c1[2], c1[1] + 8), radius=14, fill=rgba(accent, 255))
    ...
    for idx, (label, count) in enumerate(items):
        ...
        overlay = Image.new("RGBA", canvas.size, (0, 0, 0, 0))
        d = ImageDraw.Draw(overlay)
        safe_rounded_rectangle(d, (lx, ly + 4, lx + swatch, ly + 4 + swatch), radius=3, fill=rgba(color, 255))
        canvas.paste(add_overlay(canvas, overlay))
...
def compose_visualization(
    ...
    overlay = Image.new("RGBA", canvas.size, (0, 0, 0, 0))
    d = ImageDraw.Draw(overlay)
    safe_rounded_rectangle(d, (img_x, img_y, img_x + image_w, img_y + image_h), radius=18, outline=(216, 216, 216, 255), width=1)
...
def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="批量可视化 2D 框标注结果（支持三种 JSON 格式）")
    parser.add_argument(
        "--team_inputs",
        nargs="+",
        required=True,
